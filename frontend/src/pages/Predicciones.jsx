...
const banderas = {
    ...
    'Rep. Checa': 'cz', 'Haiti': 'ht', 'Bosnia y Herzegovina': 'ba',
    ...
    'Paraguay': 'py', 'Egipto': 'eg', 'Haití': 'ht',
};
...
export default function Predicciones() {
 ...
 if (tab === '16avos') return p.fase === '16avos';
  const guardar = async (p) => {
        ...
        if (res.error) {
                setMsg(prev => ({ ...prev, [p.id]: { tipo: 'error', texto: res.error } }));
                return;
        }
  ...
  return (
        <div>
              <div style={{ background: '#1D9E75', padding: '12px 16px 14px' }}>
                      <h1 style={{ color: '#E1F5EE', fontSize: 15, fontWeight: 600, margin: '0 0 2px' }}>Mis predicciones</h1>h1>
                      <p style={{ color: '#9FE1CB', fontSize: 12, margin: 0 }}>Sector las Brisas · Mundial 2026</p>p>
              </div>div>
              <div style={{ display: 'flex', background: '#0F6E56', overflowX: 'auto' }}>
                {tabs.map(t => (
                    <button key={t} onClick={() => setTab(t)} style={{
                                  padding: '9px 14px', fontSize: 11, fontWeight: 600,
                                  color: tab === t ? '#E1F5EE' : '#9FE1CB',
                                  background: 'none', border: 'none',
                                  borderBottom: tab === t ? '2px solid #E1F5EE' : '2px solid transparent',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: '#fafafa', borderBottom: '1px solid #eee' }}>
                      <span style={{ fontSize: 11, color: '#666', whiteSpace: 'nowrap' }}>{preditos} de {total} guardadas</span>span>
                      ...
                      <span style={{ fontSize: 11, color: '#666', whiteSpace: 'nowrap' }}>{pct}%</span>span>
              </div>div>
              <div className="contenido">
                {partidosFiltrados.length === 0 && (
                    <p style={{ textAlign: 'center', color: '#999', fontSize: 13, padding: '2rem 1rem' }}>Los partidos de esta fase se habilitan cuando avancen los equipos.</p>p>
                      )}
                {Object.entries(porFecha).map(([fecha, partidosDia]) => (
                    <div key={fecha}>
                                <div style={{ background: '#f0f7f4', padding: '8px 12px', borderLeft: '4px solid #1D9E75', marginBottom: 8, marginTop: 12 }}>
                                              <span style={{ fontSize: 12, fontWeight: 700, color: '#1D9E75', textTransform: 'capitalize' }}>📅 {fecha}</span>span>
                                              <span style={{ fontSize: 11, color: '#999', marginLeft: 8 }}>{partidosDia.length} partido{partidosDia.length > 1 ? 's' : ''}</span>span>
                                </div>div>
                      {partidosDia.map(p => {
                                    ...
                                    return (
                                                      <div key={p.id} className="partido-card">
                                                                        <div className="partido-header">
                                                                                            <span className="partido-info">{p.grupo} · {getHora(p.fecha_hora)}</span>span>
                                                                          {estado === 'abierto' && <span className="badge badge-green">Abierto</span>span>}
                                                                          {estado === 'pronto' && <span className="badge badge-amber">Cierra pronto</span>span>}
                                                                          {estado === 'cerrado' && <span className="badge badge-red">En juego</span>span>}
                                                                          {estado === 'finalizado' && <span className="badge badge-gray">Finalizado</span>span>}
                                                                        </div>div>
                                                        {estado === 'pronto' && (
                                                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, marginBottom: 8 }}>
                                                                                                  <span style={{ fontSize: 11, color: '#854F0B', fontWeight: 600 }}>{msRestantes(p)}</span>span>
                                                                            </div>div>
                                                                        )}
                                                                        <div className="equipos-row">
                                                                                            <div className="equipo">
                                                                                              {banderaLocal && <img src={banderaLocal} alt={p.equipo_local} style={{ width: 32, height: 22, objectFit: 'cover', borderRadius: 3, display: 'block', margin: '0 auto 4px' }} />}
                                                                                                                  <span className="equipo-nombre">{p.equipo_local}</span>span>
                                                                                              </div>div>
                                                                                            <div className="score-wrap">
                                                                                                                  ...
                                                                                                                  <span className="score-dash">—</span>span>
                                                                                            ...
                                                                                            <div className="equipo">
                                                                                              {banderaVisitante && <img src={banderaVisitante} alt={p.equipo_visitante} style={{ width: 32, height: 22, objectFit: 'cover', borderRadius: 3, display: 'block', margin: '0 auto 4px' }} />}
                                                                                                                  <span className="equipo-nombre">{p.equipo_visitante}</span>span>
                                                                                              </div>div>
                                                                        </div>div>
                                                        {editando && (
                                                        ...
                                                        {estado === 'finalizado' && p.goles_local !== null && (
                                                                            <div className="alerta alerta-ok" style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between' }}>
                                                                                                  <span>Resultado real</span>span>
                                                                                                  <strong>{p.equipo_local} {p.goles_local} – {p.goles_visitante} {p.equipo_visitante}</strong>strong>
                                                                            </div>div>
                                                                        )}
                                                      </div>div>
                                                                      );
