...
const login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.id, usuario: user.usuario, nombre_display: user.nombre_display, es_admin: user.es_admin, nombre_bloqueado: user.nombre_bloqueado, avatar: user.avatar },
      process.env.JWT_SECRET, { expiresIn: '7d' }
    );
    res.json({ token, usuario: { id: user.id, usuario: user.usuario, nombre_display: user.nombre_display, es_admin: user.es_admin, nombre_bloqueado: user.nombre_bloqueado, avatar: user.avatar } });
  } catch (err) {
    console.error(err);
...
const actualizarNombre = async (req, res) => {
  const { nombre_display, avatar } = req.body;
  ...
  try {
    ...
    await pool.query(
      'UPDATE usuarios SET nombre_display = $1, nombre_bloqueado = TRUE, avatar = $2 WHERE id = $3',
      [nombre_display.trim(), avatar || 'avatar1', req.usuario.id]
    );
    res.json({ mensaje: 'Nombre y avatar actualizados' });
  } catch (err) {
    res.status(500).json({ error: 'Error en el servidor' });
  }
};
...
const actualizarAvatar = async (req, res) => {
  ...
  try {
    await pool.query('UPDATE usuarios SET avatar = $1 WHERE id = $2', [avatar, req.usuario.id]);
...
module.exports = { login, actualizarNombre, actualizarAvatar };